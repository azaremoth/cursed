...
local barColors = {
  ...
  leveling    = { 0.10,0.10,0.10,barAlpha },  
  mana    = { 0.0,0.0,1.0,barAlpha },  
   ...
   --if (not health)    then health=-1   elseif(health<1)    then health=1    end
    ...
    --// BARS //-----------------------------------------------------------------------------
      ...
      if (health) and ((drawFullHealthBars)or(hp<1)) and ((build==1)or(hp<0.99 and (build>hp+0.01 or hp>build+0.01))or(drawFullHealthBars)) then
        ...
        if (drawFullHealthBars)or(hp100<99) then
      ...
      if (drawJumpJet)and(ci.canJump) then
        ...
        if (jumpReload and (jumpReload>0) and (jumpReload<0.99)) then
          AddBar("jump",jumpReload,"jump",(fullText and floor(jumpReload*100)..'%') or '')
        end
      end

	  --// LEVELING
      local levelingstate = GetUnitRulesParam(unitID,"xps")
      if (levelingstate and (levelingstate>0) and (levelingstate<100)) then
        AddBar("leveling",levelingstate,"leveling",(fullText and floor(levelingstate*100)..'%') or '')
      end		  
	  --// MANA
	  local manastate = GetUnitRulesParam(unitID,"mana")
	  local manaunitDefID = Spring.GetUnitDefID(unitID)
	  local unitmana = UnitDefs[manaunitDefID].customParams.mana	  
      if (manastate and unitmana and ((100*manastate/unitmana)>0) and ((100*manastate/unitmana)<100)) then
		local unitfaction = UnitDefs[manaunitDefID].customParams.factionname
		if (unitfaction == "imperials") then
			AddBar("power",(manastate/unitmana),"mana",(fullText and floor(100*manastate/unitmana)..'%') or '')
		else 
			AddBar("mana",(manastate/unitmana),"mana",(fullText and floor(100*manastate/unitmana)..'%') or '')
		end
      end
