...
function gadget:Initialize()
	...
	for _,team in ipairs(allTeams) do
		...
		if campaignBattleID then
			local customKeys = select(8, Spring.GetTeamInfo(team, true))
			if (type(customKeys) ~= "table") then customKeys = select(7, Spring.GetTeamInfo(team, true)) end --for Spring 104 compatibility			
			if customKeys then
